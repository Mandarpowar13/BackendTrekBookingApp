...
@GetMapping("/showtreks/{id}")
    ...
    public ResponseEntity<Treks> updateTrek(@PathVariable Long id ,@RequestBody Treks trek) {
     Treks exiTrek = repo.findById(id).orElse(null);
		...
		exiTrek.setDistance(trek.getDistance());
		exiTrek.setTiming(trek.getTiming());
		exiTrek.setDate(trek.getDate());
		exiTrek.setDay1(trek.getDay1());
		exiTrek.setDay2(trek.getDay2());
		exiTrek.setPar1(trek.getPar1());
		exiTrek.setPar2(trek.getPar2());
		exiTrek.setPar3(trek.getPar3());
		exiTrek.setPar4(trek.getPar4());
		exiTrek.setPar5(trek.getPar5());
